...
struct threadpool* threadpool_init(int thread_num, int queue_max_num)
{
    ...
    do
    {
        ...
        if (pthread_mutex_init(&(pool->mutex_cout), NULL))
        {
            cout<<"failed to init mutex_cout!\n";
            break;
        }
        ...
        {
            cout<<"failed to init queue_not_full!\n";
            break;
        }
        pool->queue_close = 0;
        pool->pool_close = 0;
...
void* threadpool_function(void* arg)
{
    ...
    while (1)
    {
        ...
        //pthread_mutex_lock(&(pool->mutex_cout));
        (*(pjob->callback_function))(pjob->arg);   //线程真正要做的工作，回调函数的调用
        //pthread_mutex_unlock(&(pool->mutex_cout));
